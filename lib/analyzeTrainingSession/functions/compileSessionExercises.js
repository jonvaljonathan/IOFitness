...
export const compileSessionExercises = (completedSession, progressedExerciseArray) => {
  ...
  const newExercises = completedExercises.map((completedExercise) => {
    const progressedExercise = progressedExerciseArray.find(
      (pExercise) => pExercise.exerciseName === completedExercise.exerciseName,
    );
    const isExerciseProgressed = progressedExercise !== undefined;

    return isExerciseProgressed
      ? { ...progressedExercise, complete: false }
      : { ...completedExercise, complete: false };
  });
