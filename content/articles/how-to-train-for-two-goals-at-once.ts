...
export const twoGoalsArticle: LearnArticle = {
  ...
  body: [
    ...
    {
      type: "p",
      text: "That is one reason [adaptive training](/learn/what-is-adaptive-training) matters. A useful plan should hold a primary goal, keep secondary work alive with a maintenance dose, and adjust when match day, travel, or a missed session changes the week. When the second goal is a fixed weekend sport, that becomes scheduling: [how to combine lifting with a weekend sport](/learn/how-to-combine-lifting-with-sports). A sore shoulder or angry knee is related: find a version you can still load, attack that capacity, and keep the rest of the week moving — [how to train with a limitation](/learn/how-to-train-with-a-limitation). When the week itself gets smaller, keep the priority ranking and compress on purpose: [when your week shrinks](/learn/when-your-week-shrinks). When the two goals are muscle and fat loss, the priority call gets sharper: [when building muscle and losing fat compete](/learn/when-building-muscle-and-losing-fat-compete). IOFitness is built around that allocation problem. The framework above still works with a notebook if you are honest about priority.",
    },
